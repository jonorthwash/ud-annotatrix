...
const app = express();
// render logs in console with ANSI colors
...
// extract user's IP even if the app is run behind the proxy
...
const appLogger = function (req, res, next) {
  ...
  if (cfg.environment === 'development') {
    ...
    res.on("finish", () => {
      const addrInfo = forwarded(req, req.headers);
      const elapsed = process.hrtime(started);
      const time = Math.round(elapsed[0] * 1e3 + elapsed[1] * 1e-6);
      const ip = (addrInfo.ip === "::ffff:127.0.0.1" || addrInfo.ip === "::1") ? "" : addrInfo.ip + " ";
      ...
      console.log(ip + req.method, req.originalUrl,
        colors[codeClass](res.statusCode),
        colors[timeClass](time), "ms",
        req.xhr ? colors.bold("<AJAX>") : ""
      );
    });
  }
