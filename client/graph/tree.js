...
TreeLayout.prototype.run = function() {
  ...
  let bb =
      math.makeBoundingBox(options.boundingBox ? options.boundingBox : {x1: 0, y1: 0, w: cy.width(), h: cy.height()});
  ...
  if (bb.h === 0 || bb.w === 0) {
    ...
    if (options.avoidOverlap) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        let clumpId = parseInt(nodes[i]._private.data.clump);
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        let clumpId = parseInt(node._private.data.clump);
        ...
        // console.log('tree.js: [' + i + '] nbb.w|nbb.h|cellWidth|cellHeight = ' + nbb.w +'|'+ nbb.h +'|'+ cellWidth
        // +'|'+ cellHeight);
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (rcPos && (rcPos.row !== undefined || rcPos.col !== undefined)) { // must have at least row or col def'd
        let pos = {row: rcPos.row, col: rcPos.col};
