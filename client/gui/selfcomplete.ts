// @ts-nocheck (This is a vendored file)
//
...
function Selfcomplete(el, options) {
  ...
  that.classes = {selected: "selfcomplete-selected", suggestion: "selfcomplete-suggestion"};
  that.hint = null;
  that.hintValue = "";
  that.selection = null;
...
function _formatGroup(suggestion, category) { return "<div class=\"selfcomplete-group\">" + category + "</div>"; };
...
Selfcomplete.prototype = {
  ...
  initialize: function() {
    ...
    that.noSuggestionsContainer =
        $("<div class=\"selfcomplete-no-suggestion\"></div>").html(this.options.noSuggestionNotice).get(0);
    ...
    container.on("mouseover.selfcomplete", suggestionSelector, function() { that.activate($(this).data("index")); });
    ...
    container.on("click.selfcomplete", suggestionSelector, function() { that.select($(this).data("index")); });

    container.on("click.selfcomplete", function() { clearTimeout(that.blurTimeoutId); })
  ...
  setOptions: function(suppliedOptions) {
    ...
    $(that.suggestionsContainer)
        .css({"max-height": options.maxHeight + "px", "width": options.width + "px", "z-index": options.zIndex});
  ...
  fixPosition: function() {
    ...
    var orientation = that.options.orientation, containerHeight = $container.outerHeight(),
        height = that.el.outerHeight(), offset = that.el.offset(), styles = {"top": offset.top, "left": offset.left};
  ...
  onKeyUp: function(e) {
    ...
    if (that.currentValue !== that.el.val()) {
      ...
      if (that.options.deferRequestBy > 0) {
        ...
        that.onChangeTimeout = setTimeout(function() { that.onValueChange(); }, that.options.deferRequestBy);
      } else {
        that.onValueChange();
      }
    }
  ...
  getSuggestionsLocal: function(query) {
    var that = this, options = that.options, queryLowerCase = query.toLowerCase(), filter = options.lookupFilter,
        limit = parseInt(options.lookupLimit, 10), data;
    ...
    if (response && Array.isArray(response.suggestions)) {
      ...
      that.currentRequest =
          $.ajax(ajaxSettings)
              ...
              .fail(function(
                  jqXHR, textStatus,
                  errorThrown) { options.onSearchError.call(that.element, q, jqXHR, textStatus, errorThrown); });
    } else {
  ...
  noSuggestions: function() {
    var that = this, beforeRender = that.options.beforeRender, container = $(that.suggestionsContainer),
